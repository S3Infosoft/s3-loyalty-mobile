...
//    public Spinner city;
    ...
    public void onCreate(Bundle savedInstanceState) {
       ...
       fn=findViewById(R.id.fnet);
        ...
        registeractivitytyp= findViewById(R.id.registeractivitype);

        bddate=findViewById(R.id.dobet);
        mrgdate = findViewById(R.id.annet);
        email=findViewById(R.id.emailet);
        autoram = findViewById(R.id.autocompletetextcity);
...
if(cnfpass.getText().toString().equals(pass.getText().toString())){
    cnfpass.setError(null);
        }else {cnfpass.setError("Both Passwords does not match");}
    }
...
pass.addTextChangedListener(new TextWatcher() {
    ...
    @Override
    public void onTextChanged(CharSequence s, int start, int before, int count) {
boolean chi=checkStrength(s.toString());
    }
...
ArrayAdapter<String> citiesauto = new ArrayAdapter(this,android.R.layout.select_dialog_item,citiesofindia);
autoram.setThreshold(1);
autoram.setAdapter(citiesauto);
autoram.addTextChangedListener(new TextWatcher() {
    @Override
    public void beforeTextChanged(CharSequence s, int start, int count, int after) {
    ...
    @Override
    public void onTextChanged(CharSequence s, int start, int before, int count) {
selectedcity=autoram.getText().toString();
    }
    ...
    public boolean checkStrength(String st){
        if(st.length()>7){
            int flaglen=0;
            int flagDig=0;
            int leng=st.length();
            for (char iter : st.toCharArray()) {
                if(Character.isLetter(iter)){
                    flaglen++;
                }
                else if(Character.isDigit(iter)){
                    flagDig++;
                }
            }
            if(flaglen+flagDig==leng){
//                System.out.println("You must atleast have one special Character");
//                Snackbar.make(registeractivitytyp,"You must atleast have one special Character",LENGTH_LONG).show();
                pass.setError("You must atleast have one special Character");
                return false;
            }
            else if(flagDig==0){
//                System.out.println("You must atleast have a number in your password");
                pass.setError("You must atleast have a number in your password");
//                Snackbar.make(registeractivitytyp,"You must atleast have a number in your password",LENGTH_LONG).show();
                return false;
            }else{
//                Snackbar.make(registeractivitytyp,"Great Password",LENGTH_LONG).show();
//pass.setError();
                pass.setError(null);
                return true;
            }
...
//            Snackbar.make(registeractivitytyp,"password must be atleast 8 characters long",LENGTH_LONG).show();
